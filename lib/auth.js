    ...
    @connect((state, ownProps) => {
      return {
        ...
        failureRedirectPath: typeof failureRedirectPath === 'function' ?
          failureRedirectPath(state, ownProps) : failureRedirectPath,
    ...
    class UserAuthWrapper extends Component {
      ...
      static propTypes = {
        ...
        location: shouldRedirect ?
          locationShape.isRequired : locationShape,
      ...
      render() {
        ...
        if (isAuthorized(authData)) {
          ...
          return FailureComponent ?
            <
            FailureComponent {...propMapper(this.props)
            }
          />: null
        }
      }
  ...
  if (shouldRedirect) {
    wrapComponent.onEnter = (store, nextState, replace) => {
      ...
      if (!isAuthorized(authData) && !isAuthenticating) {
        const redirectPath = typeof failureRedirectPath === 'function' ?
