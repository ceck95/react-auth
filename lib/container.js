...
* @Last modified time: 2017-03-11T11:53:31+07:00
 */
...
class ContainerBase {
  constructor(options, compose, combineReducers, createStore, applyMiddleware, syncHistoryWithStore, routerReducer, routerMiddleware, thunk, hashHistory, browserHistory) {
    ...
    let baseHistory = options.hashHistory ?
      hashHistory :
      browserHistory;
    let reducer = combineReducers(Object.assign({}, options.reducers, {
    ...
    this._statusActionLoginFail = options.authenticate.statusActionLoginFail;
  ...
  beforeAuthRender() {
    ...
    if (accessToken) {
      ...
      if (this._statusActionLoginFail) {
