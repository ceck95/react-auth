...
class ContainerBase {
  constructor(options, compose, combineReducers, createStore, applyMiddleware, syncHistoryWithStore, routerReducer, routerMiddleware, Provider, Router) {
  ...
  get varRender() {
    return <this.Provider store={this._store}>
      <this.Router history={this._history}>
        {this._routers}
      </this.Router>
    </this.Provider>,
    document.getElementById(this._elementById)
  }
